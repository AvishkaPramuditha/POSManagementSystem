...
public class OrderController {
   ...
   public boolean saveOrderDetail(ArrayList<OrderDetail> orderDetails, String orderID) throws SQLException, ClassNotFoundException {
        ...
        for (OrderDetail detail:orderDetails
            ...
            if (detail.getFoodType().equals("Meal")){table="ordermealdetail";}else if (detail.getFoodType().equals("Pizza")){table="orderpizzadetail";}else if (detail.getFoodType().equals("Sub")){table="ordersubdetail";}else if (detail.getFoodType().equals("Drink")){table=" orderdrinkdetail";}else if (detail.getFoodType().equals("Package")){table=" orderpackagedetail";}else {continue;}
    ...
    public boolean updateOrder(Order order){
        ...
        try {
            ...
            if (preparedStatement.executeUpdate()>0){
                if (updateOrderDetail(order.getOrderID(),order.getOrderedItem())){ connection.commit();return true;}else{connection.rollback();return false;}
            }else{
                connection.rollback();return false;}
    ...
    private boolean updateOrderDetail(String orderID,ArrayList<OrderDetail> orderDetailArrayList) throws SQLException, ClassNotFoundException {
        ...
        for (OrderDetail detail:orderDetailArrayList
            ...
            if (detail.getFoodType().equals("Meal")){table="ordermealdetail";id="MealID";}else if (detail.getFoodType().equals("Pizza")){table="orderpizzadetail";id="PizzaID";}else if (detail.getFoodType().equals("Sub")){table="ordersubdetail";id="SandwichID";}else if (detail.getFoodType().equals("Drink")){table=" orderdrinkdetail";id="BeverageID";}else if (detail.getFoodType().equals("Package")){table=" orderpackagedetail";id="PackageID";}else {continue;}
